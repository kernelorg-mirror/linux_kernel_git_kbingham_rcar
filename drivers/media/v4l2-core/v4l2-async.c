...
static bool v4l2_async_is_notifier_complete(struct v4l2_async_notifier *notifier)
{
	struct v4l2_subdev *sd, *tmp;
...
static int v4l2_async_try_complete_notifier(struct v4l2_async_notifier *notifier)
{
	struct v4l2_async_notifier *root = notifier;

	while (root->subnotifier) {
		root = subnotifer_to_v4l2_subdev(root)->notifier;
		/* No root notifer can be found at this time */
		if (!root)
			return 0;
	}
				  ...
				  struct v4l2_async_subdev *asd)
{
	...
	/* Register the subnotifer if it's not empty */
	if (sd->subnotifier.num_subdevs) {
