...
static int adv7482_pad_from_fwnode(struct fwnode_endpoint *endpoint,
			       unsigned int *pad)
{
	if (endpoint->port >= ADV7482_PAD_MAX)
		return -ENXIO;
...
static const struct media_entity_operations adv7482_media_ops = {
	.pad_from_fwnode = adv7482_pad_from_fwnode,
...
MODULE_DEVICE_TABLE(of, adv7482_of_table);
