				     ...
				     struct v4l2_async_subdev *asd)
{
	...
	for (i = 0; i < RCAR_VIN_NUM; i++)
		if (vin->group->vin[i])
			rvin_v4l2_remove(vin->group->vin[i]);
...
static int rvin_group_init(struct rvin_dev *vin)
{
	...
	/*
	 * Check number of registerd VIN in group against the group mask.
	 * If the mask is empty DT have not yet been parsed and if the
	 * count match all VINs are registerd and it's safe to register
	 * the async notifier
	 */
	mutex_lock(&vin->group->lock);
